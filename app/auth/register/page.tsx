...
export default function Register() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (password !== confirmPassword) {
      setValidationError(t._('auth.validation.passwordMismatch'));
      return;
    }

    if (password.length < 8) {
      setValidationError(t._('auth.validation.passwordTooShort'));
      return;
    }
    ...
    <div className="min-h-screen flex flex-col items-center justify-center p-4 transition-colors dark:bg-gray-800">
      ...
      <div className="w-full max-w-md rounded-lg p-8 transition-colors">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            <label htmlFor="confirmPassword" className="mb-1 block text-sm font-medium text-gray-700 dark:text-gray-300">
              {t._('auth.fields.confirmPassword')}
            </label>
            <input
              id="confirmPassword"
              type="password"
          ...
          >
            {loading ? 'Loading...' : t._('auth.register.button')}
          </button>
        </form>
        ...
        <div className="mt-6 text-center text-sm">
          <span className="text-gray-600 dark:text-gray-400">{t._('auth.login.prompt')} </span>
